...
class OpenAIResponsesProvider:
    ...
    def _extract_text(data: dict[str, Any]) -> str:
        ...
        if not text:
            raise RuntimeError("model provider returned no text output")
        return text
...
class OpenAICompatibleProvider:
    """Backward-compatible chat-completions provider for other gateways."""
...
class ModelRouter:
    ...
    def _from_environment() -> ModelProvider:
        ...
        if provider == "openai":
            base_url = os.getenv("AETHON_MODEL_BASE_URL", "https://api.openai.com/v1")
            model = os.getenv("AETHON_MODEL_NAME", "gpt-5.6-luna")
            api_key = os.getenv("AETHON_MODEL_API_KEY", "")
            if not api_key:
                raise RuntimeError("AETHON_MODEL_API_KEY is required when AETHON_MODEL_PROVIDER=openai")
