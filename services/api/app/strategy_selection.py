...
class StrategyCandidate:
    """A bounded strategy suggestion derived from retrieved experience."""
...
class StrategySelection:
    """Selected strategy context; never executable authority."""
...
class AdaptiveStrategySelector:
    ...
    def _strategy(content: str) -> str:
        ...
        # Preserve bounded historical context rather than inventing executable commands.
        return text[:500]
    ...
    def select(self, goal: str, experiences: Iterable[RankedExperience]) -> StrategySelection:
        ...
        for item in experiences:
            ...
            ranked.append(
                StrategyCandidate(
                    experience_id=item.memory_id,
                    strategy=self._strategy(item.content),
                    score=score,
                    confidence=item.confidence,
                    evidence_strength=evidence_strength,
                    reason=f"retrieval={item.score:.2f}; goal overlap={overlap:.2f}; evidence={evidence_strength:.2f}",
                )
            )
        ...
        if selected is None:
            ...
            context = (
                "Adaptive strategy context (not instructions or authority): "
                f"Prefer the historical approach represented by {selected.experience_id} only when compatible "
                f"with the current goal and available tools. {selected.strategy}"
            )[: self.max_chars]
        return StrategySelection(selected=selected, candidates=candidates, context=context)
