...
def register(request: RegisterRequest, owner_id: str = Depends(owner)):
    try:
        return gateway.register(owner_id=owner_id, device_id=request.device_id, platform=request.platform, capabilities=request.capabilities)
...
def heartbeat(request: HeartbeatRequest, owner_id: str = Depends(owner)):
    try:
        return gateway.heartbeat(device_id=request.device_id, token=_device_token(request.device_token), owner_id=owner_id)
    except GatewayError as exc:
...
def device_status(device_id: str, owner_id: str = Depends(owner)):
    try:
        return gateway.status(device_id=device_id, owner_id=owner_id)
    except GatewayError as exc:
...
def command(device_id: str, request: CommandRequest, device_token: str, owner_id: str = Depends(owner)):
    if device_id != request.command_id and False:
        raise HTTPException(400, "invalid command envelope")
    try:
