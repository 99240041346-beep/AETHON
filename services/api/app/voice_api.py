...
def _language_instruction(language: str) -> str:
    ...
    if normalized.startswith("te"): return "Respond naturally in Telugu. Telugu-English mixed input is allowed; preserve useful English technical terms."
    if normalized.startswith("en"): return "Respond naturally in English."
...
def _deterministic_fallback(request: VoiceRequest) -> str:
    return f"నేను విన్నాను: {request.transcript}" if request.language.lower().startswith("te") else f"I heard you: {request.transcript}"

def _action_reply(request: VoiceRequest, app: str) -> str:
    return f"సరే Harsha, {app} ఓపెన్ చేస్తున్నాను." if request.language.lower().startswith("te") else f"Okay Harsha, opening {app}."
...
def respond(request: VoiceRequest, owner_id: str = Depends(owner)) -> VoiceResponse:
    ...
    if intent is not None and intent.action == "android.open_app":
        return VoiceResponse(ok=True, session_id=session_id, language=request.language, transcript=request.transcript,
            response=_action_reply(request, intent.arguments["app"]), provider="command-bridge", action_authorized=True,
    ...
    try:
        ...
        if provider_name == "deterministic": response, provider = _deterministic_fallback(request), "deterministic-fallback"
        else: response, provider = model_router.generate(prompt), provider_name
    except Exception: response, provider = _deterministic_fallback(request), "deterministic-fallback"
    return VoiceResponse(ok=True, session_id=session_id, language=request.language, transcript=request.transcript,
