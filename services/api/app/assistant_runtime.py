...
class AssistantRuntime:
    ...
    def _tool(self, request: ToolRequest, request_id: str, events: list[RuntimeEvent], require_approval: bool) -> RuntimeResult | None:
        ...
        if spec is None:
            return RuntimeResult(request_id, "", AssistantMode.CHAT, self.orchestrator.classify("tool"), "Tool not found.", events=tuple(events), error="tool not found")
        ...
        try:
            ...
            return RuntimeResult(request_id, "", AssistantMode.CHAT, self.orchestrator.classify("tool"), "This action was blocked by AETHON's safety policy.", events=tuple(events), error=str(exc))
        ...
        return RuntimeResult(request_id, "", AssistantMode.CHAT, self.orchestrator.classify("tool"), response, result, tuple(events), verified=result.verified, action_authorized=bool(result.ok), error=result.error if not result.ok else None)
    ...
    def run(
        ...
        *,
        ...
        if intent.mode is AssistantMode.ACTION:
            ...
            self.repository.add_message(session_id, owner_id, "assistant", response, language, intent=intent.action, action=intent.action, status="AWAITING_APPROVAL" if intent.requires_confirmation else "PLANNED", metadata={"request_id": request_id, "verified": False})
        ...
        if tool is not None and execute_tools:
            ...
            result = self._tool(ToolRequest(tool=tool_name, arguments=arguments, request_id=UUID(request_id)), request_id, events, require_approval)
            if result is not None:
                result = RuntimeResult(result.request_id, session_id, intent.mode, intent, result.response, result.tool_result, result.events, result.verified, result.action_authorized, result.requires_confirmation, result.error)
                status = "VERIFIED" if result.verified else ("SUCCEEDED" if result.tool_result and result.tool_result.ok else "FAILED")
                self.repository.add_message(session_id, owner_id, "tool", result.response, language, intent=tool_name, status=status, metadata={"request_id": request_id, "verified": result.verified})
                self.repository.add_message(session_id, owner_id, "assistant", result.response, language, intent=tool_name, status=status, metadata={"request_id": request_id, "verified": result.verified})
                return result
        ...
        self.repository.add_message(session_id, owner_id, "assistant", response, language, status=status, metadata={"request_id": request_id, "verified": False})
