...
public final class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private AndroidCommandPoller commandPoller;
    ...
    @Override protected void onCreate(Bundle state) { super.onCreate(state); buildUi(); commandPoller=new AndroidCommandPoller(this,new AndroidCommandPoller.Listener(){public void onStatus(String s){status.setText(s);}public void onAction(AndroidActionExecutor.Result r,String id){addMessage("⚙ Device",id+" • "+r.message+(r.verified?" • VERIFIED":" • NOT VERIFIED"));}}); textToSpeech=new TextToSpeech(this,this); if(Build.VERSION.SDK_INT>=23&&checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.RECORD_AUDIO},REQUEST_RECORD_AUDIO);else initSpeech(); }
    ...
    private void connectDevice(){String base=apiUrl.getText().toString().trim().replaceAll("/+$","");String id=deviceIdInput.getText().toString().trim();String token=deviceTokenInput.getText().toString();if(base.isEmpty()||id.isEmpty()||token.length()<20){status.setText("Cloud URL, device ID and valid device token are required");return;}try{commandPoller.start(base,id,token);status.setText("Device link starting • authenticated polling");}catch(Exception e){status.setText("Device link rejected • invalid configuration");}}
    ...
    @Override protected void onDestroy(){if(commandPoller!=null)commandPoller.close();if(speechRecognizer!=null)speechRecognizer.destroy();if(textToSpeech!=null){textToSpeech.stop();textToSpeech.shutdown();}super.onDestroy();}
}
