...
public final class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private void buildUi() {
        ...
        TextView ready=label("●  AETHON READY",13);ready.setTextColor(Color.rgb(74,222,128));sc.addView(ready);status=label("Online cloud • Offline device controls available",14);status.setTextColor(Color.rgb(203,213,225));sc.addView(status);
        ...
        Button ask=button("➤  Ask AETHON");ask.setOnClickListener(v->sendAssistant());LinearLayout.LayoutParams ap=new LinearLayout.LayoutParams(0,dp(50),1);ap.setMargins(dp(8),0,0,0);row.addView(ask,ap);
    ...
    private void startListening(){if(speechRecognizer==null)initSpeech();if(speechRecognizer==null||speaking)return;Intent i=new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH);i.putExtra(RecognizerIntent.EXTRA_LANGUAGE,"te-IN");i.putExtra(RecognizerIntent.EXTRA_LANGUAGE_PREFERENCE,"te-IN");i.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL,RecognizerIntent.LANGUAGE_MODEL_FREE_FORM);i.putExtra(RecognizerIntent.EXTRA_PARTIAL_RESULTS,false);speechRecognizer.startListening(i);}
    private void toggleVoiceMode(Button button){voiceMode=!voiceMode;button.setText(voiceMode?"⏹  Voice control ON":"🎙  Voice control");if(voiceMode){status.setText("Voice control enabled");startListening();}else{if(speechRecognizer!=null)speechRecognizer.stopListening();status.setText("Voice control paused");}}
