...
public final class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private void connectDevice(){String id=deviceIdInput.getText().toString().trim();String token=deviceTokenInput.getText().toString();if(id.isEmpty()||token.length()<20){status.setText("Device ID and valid device token are required");return;}try{AndroidCommandService.start(this,DEFAULT_CLOUD_URL,id,token);status.setText("Device link starting • local/offline actions remain available");}catch(Exception e){status.setText("Device link rejected • invalid configuration");}}
    ...
    private void selectPreferredVoice(){if(textToSpeech==null)return;Locale locale=Locale.forLanguageTag(responseLocale);List<Voice> voices=textToSpeech.getVoices();Voice best=null;for(Voice v:voices){if(v==null||v.getLocale()==null||!v.getLocale().getLanguage().equals(locale.getLanguage()))continue;String n=v.getName().toLowerCase(Locale.ROOT);if(n.contains("female")||n.contains("woman")||n.contains("femin")){best=v;break;}if(best==null&&!v.isNetworkConnectionRequired())best=v;}if(best==null){for(Voice v:voices){if(v!=null&&v.getLocale()!=null&&v.getLocale().getLanguage().equals(locale.getLanguage())){best=v;break;}}}if(best!=null)textToSpeech.setVoice(best);}
